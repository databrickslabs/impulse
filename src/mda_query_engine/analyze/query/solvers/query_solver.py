...
class QuerySolver(ABC):
        ...
        return spark.createDataFrame(
            ...
            schema=T.StructType(
                [
                    ...
                    T.StructField(
                        "selector_ids", T.ArrayType(T.IntegerType())
                    ),
                ]
            ),
        )
    ...
    def filter_container_metrics(
        ...
        Must return a DataFrame that includes **all columns** needed for
        container dimensions and event bounds (e.g. ``container_id``,
        ``start_ts``/``stop_ts`` or ``start_dt``/``stop_dt``), not only
        ``container_id``.
    ...
    def filter_channel_metrics(
        self, spark, db: MeasurementDB, channel_df, selectors
    ) -> DataFrame:
        """
