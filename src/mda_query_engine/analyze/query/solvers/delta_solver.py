...
class DeltaSolver(QuerySolver):
    ...
    def filter_container_tags(self, spark, query) -> DataFrame:
        ...
        tags = self._apply_column_mapping(
            tags, self.config.container_tags.column_name_mapping
        )
        # apply filters
        if len(filters) > 0:
        ...
        if len(filters) > 0:
            ...
            if pre_filtered_containers_df is not None:
                ...
                metrics = self._apply_column_mapping(
                    query.db.container_metrics(spark), cm_mapping
                )
            container_ids = container_df.select("container_id").distinct()
            return metrics.join(F.broadcast(container_ids), on=["container_id"], how="inner")
    ...
    def filter_channel_tags(self, spark, db, container_df, selectors) -> DataFrame:
        ...
        tbl = self._apply_column_mapping(
            tbl, self.config.channel_tags.column_name_mapping
        )
        ...
        tags = tags.withColumn(
            "selector_id", self._build_selector_id_expr(selectors)
        )
    ...
    def filter_channel_metrics(self, spark, db, channel_df, selectors) -> DataFrame:
        ...
        tbl = self._apply_column_mapping(
            tbl, self.config.channel_metrics.column_name_mapping
        )
        ...
        metrics = metrics.withColumn(
            "selector_ids", F.array(F.col("selector_id"))
        ).drop("selector_id")
        return metrics
    ...
    def solve(self, query, channels_df, selections, dtypes):
        ...
        q = self._apply_column_mapping(
            q, self.config.channels.column_name_mapping
        )
