"""CalculatedChannel aggregation for computing derived time-series channels."""
...
class CalculatedChannel(Aggregation):
    ...
    or ``rpm + speed``).  When evaluated per container the wrapped expression must
    yield a :class:`SampleSeries`; :meth:`QueryBuilder.solve_calculated_channels`
    explodes that series into narrow rows matching the silver ``channel_data``
    shape (``container_id, channel_id, tstart, tend, value``) plus one string
    column per identity entry.
    ...
    def dtype(self) -> T.DataType:
        """Spark type of the emitted ``value`` column (matches CHANNELS_SCHEMA)."""
        return T.DoubleType()
