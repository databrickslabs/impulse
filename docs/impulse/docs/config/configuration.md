...
| `max_selectors_per_batch` | `int`        | `500`                    | Maximum number of selectors solved per batch. Applies to events, aggregations, and calculated channels. The old name `batch_size` is a **deprecated alias** (still accepted, warns, removed in a future release); setting both raises a validation error. |
