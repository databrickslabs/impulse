...
| `max_selectors_per_batch` | `int`        | `500`                    | Maximum number of unique selectors solved per batch. Applies to events, aggregations, and calculated channels. `batch_size` is a **deprecated alias** kept for backward compatibility; it still works but emits a `DeprecationWarning` and will be removed in a future release. Setting both `max_selectors_per_batch` and `batch_size` is rejected. |
