...
def test_batched_calculated_channels_union(spark):
    """max_selectors_per_batch=1 with two channels on DISTINCT input selectors splits them
    into separate batches (each persisted as a temp table); the final fact is the union.
    """
    report = Report(
        name="calc_channel_report",
        spark=spark,
