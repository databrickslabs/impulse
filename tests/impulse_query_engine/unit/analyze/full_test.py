...
def test_full(spark, narrow_db):
    ...
    df = query.select(expr1, expr2).toPandas(spark, solver=DefaultSolver(spark))
    assert len(df) == 1
...
def test_basic_narrow_full(spark, basic_narrow_db):
    ...
    df = query.select(expr1, expr2).solve(spark, solver=DefaultSolver(spark))
    assert df.count() == 3
...
def test_sample_series_returned_as_sample_series(spark, basic_narrow_db):
    ...
    pdf = query.select(expr).toPandas(spark, solver=DefaultSolver(spark))
    assert len(pdf) == 3
...
def test_sample_series_solve_returns_binary(spark, basic_narrow_db):
    ...
    df = query.select(expr).solve(spark, solver=DefaultSolver(spark))
    assert df.count() == 3
...
def test_sample_series_mixed_with_scalar(spark, basic_narrow_db):
    ...
    pdf = query.select(expr_series, expr_max).toPandas(spark, solver=DefaultSolver(spark))
    assert len(pdf) == 3
