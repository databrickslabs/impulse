...
from tests.conftest import setup_key_value_store_db, setup_narrow_db, spark
...
def test_simple_report1(spark, setup_narrow_db):
    ...
    my_report = Report(name="my_report", spark=spark, config=dict(extended_mda_config))
...
def test_basic_narrow_report_no_uuts(spark):
    ...
    my_report: Report = Report(name="my_report", spark=spark, config=dict(config))
...
def test_basic_narrow_report(spark):
    ...
    my_report: Report = Report(name="my_report", spark=spark, config_path=config_path)
...
def test_persist_report_with_events(spark):
    ...
    tables = spark.catalog.listTables("spark_catalog.gold")
...
def test_simple_report_key_value_store(spark, key_value_store_db):
    ...
    Aligned with config_cs_bronze.json: uses KeyValueStoreSolver
    ...
    my_report = Report(name="my_report", spark=spark, config=dict(config))
