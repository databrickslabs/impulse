...
def _build_report(spark, config_dict=None):
    ...
    with (
        ...
        report = Report(name="test_report", spark=spark, config=config_dict)
...
class TestDetermineReportFullMode:
    ...
    @pytest.fixture(autouse=True)
    def mock_batched_solve(self):
        """Mock batched solve and cleanup so tests don't need Spark tables."""
        with (
            patch.object(Report, "_cleanup_temp_tables"),
            patch.object(Report, "_solve_expressions_batched", return_value=None),
        ):
            yield
    ...
    def test_full_mode_passes_none_as_pre_filtered(self, spark):
        """In full mode, pre_filtered_containers_df=None is passed to dispatch_events."""
        report = _build_report(spark)

        mock_event1 = MagicMock()
        mock_event1.get_expression.return_value = MagicMock()
        mock_event1.get_id.return_value = 1

        events_by_type = {"BASIC_EVENT": [mock_event1]}

        mock_dispatch_result = {"BASIC_EVENT": MagicMock(spec=DataFrame)}
        ...
        with (
            ...
            patch(
                "mda_reporting.core.report.split_by_hash_change",
                return_value=(
                    {"BASIC_EVENT": [mock_event1]},
                    {},
                    {"BASIC_EVENT": [1]},
                ),
            ),
            patch("mda_reporting.core.report.collect_solvable_expressions", return_value=[]),
            patch(
                "mda_reporting.core.report.dispatch_events",
                return_value=mock_dispatch_result,
            ) as mock_dispatch,
            patch("mda_reporting.core.report.dispatch_aggregations", return_value={}),
            patch.dict(
                EventType._member_map_,
                {"BASIC_EVENT": MagicMock(name="BASIC_EVENT")},
            ),
        ):
            ...
            # dispatch_events is called twice: once for changed (None), once for unchanged
            assert mock_dispatch.call_count == 2
            # First call: changed events with pre_filtered_containers_df=None
            first_call = mock_dispatch.call_args_list[0]
            assert first_call.args[6] is None  # pre_filtered_containers_df=None

    def test_full_mode_events_stored_as_dataframe_not_dict(self, spark):
        """In full mode, event_dfs values are dicts with 'changed'/'unchanged' DataFrame entries."""
        report = _build_report(spark)
...
class TestDetermineReportIncrementalMode:
    ...
    def test_incremental_detect_returns_none_falls_back_to_full(self, spark):
        ...
        with (
            ...
            # First dispatch call (changed) should have pre_filtered=None
            first_call = mock_dispatch.call_args_list[0]
            assert first_call.args[6] is None
            # Stored as DataFrame, not dict (the "changed" value is a DataFrame)
            assert not isinstance(report.event_dfs["BASIC_EVENT"]["changed"], dict)
    ...
    def test_incremental_changed_events_processed_with_none(self, spark):
        ...
        mock_changed_event_df = MagicMock(spec=DataFrame)
        mock_unchanged_event_df = MagicMock(spec=DataFrame)
        ...
        with (
            ...
            patch(
                "mda_reporting.core.report.split_by_hash_change",
                return_value=(
                    {"BASIC_EVENT": [changed_event]},
                    {"BASIC_EVENT": [unchanged_event]},
                    {"BASIC_EVENT": [42]},
                ),
            ),
            patch(
                "mda_reporting.core.report.collect_solvable_expressions",
                return_value=[],
            ),
            patch(
                "mda_reporting.core.report.dispatch_events",
                side_effect=[
                    {"BASIC_EVENT": mock_changed_event_df},
                    {"BASIC_EVENT": mock_unchanged_event_df},
                ],
            ) as mock_dispatch,
            patch(
                "mda_reporting.core.report.dispatch_aggregations",
                return_value={},
            ),
            patch.dict(
                EventType._member_map_,
            ...
            # dispatch_events called twice: changed events (None) + unchanged (mock_upserted_df)
            assert mock_dispatch.call_count == 2

            # First call: changed events with pre_filtered_containers_df=None
            first_call = mock_dispatch.call_args_list[0]
            assert first_call.args[6] is None  # pre_filtered_containers_df

            # Second call: unchanged events with pre_filtered_containers_df=mock_upserted_df
            second_call = mock_dispatch.call_args_list[1]
            assert second_call.args[6] is mock_upserted_df  # pre_filtered_containers_df
    ...
    def test_incremental_changed_event_ids_tracked(self, spark):
        ...
        with (
            ...
            patch("mda_reporting.core.report.collect_solvable_expressions", return_value=[]),
            patch("mda_reporting.core.report.dispatch_events", return_value={}),
            patch("mda_reporting.core.report.dispatch_aggregations", return_value={}),
            patch.dict(
                EventType._member_map_,
                {"BASIC_EVENT": MagicMock(value=mock_event_cls, name="BASIC_EVENT")},
    ...
    def test_incremental_no_changed_events_all_unchanged(self, spark):
        ...
        with (
            ...
            patch(
                ...
                return_value=(
                    {},  # No changed events
                    {"BASIC_EVENT": [event]},  # All unchanged
                    {},  # No changed IDs
                ),
            ),
            patch("mda_reporting.core.report.collect_solvable_expressions", return_value=[]),
            patch(
                "mda_reporting.core.report.dispatch_events",
                side_effect=[
                    {},  # Changed dispatch: empty
                    {"BASIC_EVENT": mock_unchanged_df},  # Unchanged dispatch
                ],
            ) as mock_dispatch,
            patch("mda_reporting.core.report.dispatch_aggregations", return_value={}),
            patch.dict(
            ...
            # dispatch_events called twice: changed (empty) + unchanged (with pre_filtered)
            assert mock_dispatch.call_count == 2
            # Second call: unchanged with pre_filtered_containers_df=mock_upserted_df
            second_call = mock_dispatch.call_args_list[1]
            assert second_call.args[6] is mock_upserted_df
