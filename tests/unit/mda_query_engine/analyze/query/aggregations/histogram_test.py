...
def test_histogram_custom_weight_str():
    ...
    expected_str = "<Histogram channel=TimeSeriesSelector<TagOp<eq(TagSelector<name>,test)>>, weights=TimeSeriesSelector<TagOp<eq(TagSelector<name>,test)>>, bins=[0.0, 1.0, 2.0], channel_interp_kind=previous, weights_interp_kind=previous, math_fct_for_weights=None, math_fct_kwargs={}>"
    assert str(hist) == expected_str
